-- 0) Secure ai_processing_stats (it's a VIEW): revoke direct access; use secured RPC instead
DO $$ BEGIN
  -- Revoke from PUBLIC to cover anon/authenticated; then explicitly from anon/authenticated
  BEGIN
    REVOKE ALL ON public.ai_processing_stats FROM PUBLIC;
  EXCEPTION WHEN undefined_table THEN
    -- View may not exist in some environments; ignore
    NULL;
  END;
  BEGIN
    REVOKE ALL ON public.ai_processing_stats FROM anon;
  EXCEPTION WHEN undefined_object THEN
    NULL;
  END;
  BEGIN
    REVOKE ALL ON public.ai_processing_stats FROM authenticated;
  EXCEPTION WHEN undefined_object THEN
    NULL;
  END;
END $$;

-- Ensure RPC is callable by authenticated users (already SECURITY DEFINER with checks)
GRANT EXECUTE ON FUNCTION public.get_ai_processing_stats(uuid) TO authenticated;


-- 1) Lock down association_users to prevent self-escalation
DO $$ BEGIN
  IF EXISTS (
...
-- 2) Harden assign_user_to_association with authorization checks
...
-- 3) Prevent role changes on profiles by non-admins via trigger
