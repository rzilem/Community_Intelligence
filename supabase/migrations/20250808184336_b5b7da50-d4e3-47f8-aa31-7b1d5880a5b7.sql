-- Accounting Views Migration (drop-then-create)
BEGIN;

-- v_trial_balance
DO $$
BEGIN
  -- Always drop first to avoid column-change errors
  IF to_regclass('public.v_trial_balance') IS NOT NULL THEN
    EXECUTE 'DROP VIEW IF EXISTS public.v_trial_balance';
  END IF;
...
DO $$
BEGIN
  IF to_regclass('public.v_income_statement') IS NOT NULL THEN
    EXECUTE 'DROP VIEW IF EXISTS public.v_income_statement';
  END IF;
...
DO $$
BEGIN
  IF to_regclass('public.v_balance_sheet') IS NOT NULL THEN
    EXECUTE 'DROP VIEW IF EXISTS public.v_balance_sheet';
  END IF;
