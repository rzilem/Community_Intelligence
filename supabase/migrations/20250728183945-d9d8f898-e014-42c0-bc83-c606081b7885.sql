-- Phase 3-6: Create missing tables (Fixed version)
...
-- Lead Follow-up Sequences Table (using leads table as is)
...
-- Simple policy for lead follow-up sequences (users can access their own leads)
CREATE POLICY "Users can access their own lead follow-up sequences" ON public.lead_follow_up_sequences
  FOR ALL USING (lead_id IN (SELECT id FROM public.leads));
...
CREATE INDEX IF NOT EXISTS idx_work_orders_association_id ON public.work_orders(association_id);
CREATE INDEX IF NOT EXISTS idx_work_orders_property_id ON public.work_orders(property_id);
CREATE INDEX IF NOT EXISTS idx_work_orders_status ON public.work_orders(status);

CREATE INDEX IF NOT EXISTS idx_inspections_association_id ON public.inspections(association_id);
CREATE INDEX IF NOT EXISTS idx_inspections_property_id ON public.inspections(property_id);
CREATE INDEX IF NOT EXISTS idx_inspections_status ON public.inspections(status);

CREATE INDEX IF NOT EXISTS idx_lead_follow_up_sequences_lead_id ON public.lead_follow_up_sequences(lead_id);
CREATE INDEX IF NOT EXISTS idx_lead_follow_up_sequences_status ON public.lead_follow_up_sequences(status);

CREATE INDEX IF NOT EXISTS idx_resale_certificates_association_id ON public.resale_certificates(association_id);
CREATE INDEX IF NOT EXISTS idx_resale_certificates_property_id ON public.resale_certificates(property_id);
CREATE INDEX IF NOT EXISTS idx_resale_certificates_status ON public.resale_certificates(status);

CREATE INDEX IF NOT EXISTS idx_transfer_requirements_association_id ON public.transfer_requirements(association_id);
CREATE INDEX IF NOT EXISTS idx_document_templates_association_id ON public.document_templates(association_id);