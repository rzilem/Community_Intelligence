...
function processEmail(emailData: any): {
  name: string;
  email: string;
  company?: string;
  phone?: string;
  ...
  try {
    ...
    if (emailData.text) {
      ...
      // If no plain text, use the HTML body but note that it's HTML
      notes += "(HTML content) " + emailData.html.replace(/<[^>]*>/g, ' ').substring(0, 500);
    }
...
serve(async (req) => {
  ...
  try {
    ...
    const leadData = processEmail(emailData);
