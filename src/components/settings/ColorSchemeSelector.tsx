...
const ColorSchemeSelector: React.FC<ColorSchemeSelectorProps> = ({ value, onChange }) => {
  const colorSchemes: { value: ColorScheme; label: string; color: string }[] = [
    { value: 'default', label: 'Default Blue', color: 'bg-blue-600' },
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {colorSchemes.map((scheme) => (
        <div key={scheme.value} className="flex flex-col items-center gap-1">
          <button
            type="button"
            className={cn(
              "relative rounded-full overflow-hidden h-10 w-10 flex items-center justify-center border-2 transition-all",
              value === scheme.value ? "border-primary scale-110" : "border-transparent hover:scale-105"
            )}
