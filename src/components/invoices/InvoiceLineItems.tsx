...
export const InvoiceLineItems: React.FC<InvoiceLineItemsProps> = ({
  ...
  const handleLineChange = (index: number, field: string, value: string | number) => {
    ...
    if (field === 'amount' && index > 0) {
      const totalExcludingFirst = newLines.slice(1).reduce((sum, line) => sum + (Number(line.amount) || 0), 0);
  ...
  return <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
      <div className="flex justify-between items-center border-b pb-4">
        ...
        <Button variant="outline" size="sm" onClick={handleAddLine} className="flex items-center gap-2 text-green-600 hover:bg-green-50">
      ...
      {lines.map((line, index) => <div key={index} className="relative space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200 grid grid-cols-12 gap-4">
          <div className="col-span-1 flex items-center justify-center px-[12px] mx-[29px] my-[76px] py-[4px]">
          ...
          <div className="col-span-11 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Select value={line.glAccount || "none"} onValueChange={value => handleLineChange(index, 'glAccount', value)}>
              ...
              <div className="space-y-2">
                ...
                <Select value={line.fund} onValueChange={value => handleLineChange(index, 'fund', value)}>
                  <SelectTrigger className="w-full">
              ...
              <div className="space-y-2">
                ...
                <Select value={line.bankAccount} onValueChange={value => handleLineChange(index, 'bankAccount', value)}>
                  <SelectTrigger className="w-full">
            ...
            <div className={showPreview ? 'space-y-4' : 'grid grid-cols-5 gap-4'}>
              <div className={showPreview ? 'w-full' : 'col-span-4'}>
                ...
                <Input id={`description-${index}`} value={line.description} onChange={e => handleLineChange(index, 'description', e.target.value)} placeholder="Enter description" className="mt-2" />
              </div>
              <div>
                <Label htmlFor={`amount-${index}`} className="text-sm font-medium text-gray-600">
                  Amount
                </Label>
                <Input id={`amount-${index}`} type="number" value={line.amount} onChange={e => handleLineChange(index, 'amount', parseFloat(e.target.value) || 0)} placeholder="0.00" className="mt-2 text-right" step="0.01" disabled={index === 0} />
              </div>
            </div>
          </div>
        </div>)}
