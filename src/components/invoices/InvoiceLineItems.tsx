...
export const InvoiceLineItems: React.FC<InvoiceLineItemsProps> = ({
  ...
  const handleAddLine = () => {
    // Get fund and bankAccount from the first line
    const { fund, bankAccount } = lines[0];
    
    onLinesChange([...lines, {
      glAccount: '',
      fund: fund, // Carry over fund from first line
      bankAccount: bankAccount, // Carry over bankAccount from first line
      description: '',
      amount: 0
    }]);
  };
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
      ...
      {lines.map((line, index) => (
        <div key={index} className="relative space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200 grid grid-cols-12 gap-4">
